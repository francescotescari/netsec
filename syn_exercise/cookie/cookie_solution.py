...
def encode_sequence_number(ip: str, port: int, minute_timestamp: int, mss_index: int) -> int:
    ...
    hash_num = uint32_hash_fields(ip, port, minute_timestamp)
    # 2) Encode 24bits of the hash, the mss_index and timestamp lowest bits in a unique 32bit number
    lowest_bits = minute_timestamp & 0b111111
    sequence_number = hash_num & 0xffffff
    sequence_number = (mss_index << 24) + sequence_number
    sequence_number = (lowest_bits << 26) + sequence_number
    # 4) Encrypt (xor with the hash of) the sequence number with the source ip and port
    # Q) How hard it is for an attacker to forge a valid cookie, given a source IP and port? Assume MAX_SYNCOOKIE_AGE=2
    # A) The message looks like random, the attacker have a chance of 1/2^29 to forge a valid cookie:
    #    2/
    sequence_number = sequence_number ^ uint32_hash_fields(ip, port)
...
def decode_sequence_number(sequence_number: int, ip: str, port: int, minute_timestamp: int) -> int:
    ...
    origin_lowest_bits = sequence_number >> 26
    current_lowest_bits = minute_timestamp & 0b111111
    time_passed = (current_lowest_bits - origin_lowest_bits) & 0b111111
    # 3) Verify that the difference is lower than the MAX_SYNCOOKIE_AGE
    if time_passed >= MAX_SYNCOOKIE_AGE:
        raise InvalidAck("This cookie is expired")
    # 4) Calculate the hash value of the ip, port and original timestamp (not the current one)
    original_timestamp = minute_timestamp - time_passed
    hash_num = uint32_hash_fields(ip, port, original_timestamp)
    # 5) Verify that calculated hash is equal to the hash encoded in the sequence number
    if (hash_num & 0xffffff) != (sequence_number & 0xffffff):
        raise InvalidAck("Invalid cookie signature")
    # 6) Decode and return the MSS index
    return (sequence_number >> 24) & 0b11
...
def encode_options(timestamp: int, window_scaling: int, sack: bool, ecn: bool) -> int:
    ...
    timestamp = 0b100000 * ecn + timestamp
    timestamp = 0b010000 * sack + timestamp
    timestamp = (0b001111 & window_scaling) + timestamp
    return timestamp
...
def decode_options(timestamp: int) -> (int, int, bool, bool):
    ...
    E=ecn, S=sack, W=window_scaling"""
    ecn = timestamp & 0b100000
    sack = timestamp & 0b010000
    window_scaling = timestamp & 0b001111
    return timestamp, window_scaling, sack, ecn
